...
func TestRxWindow(t *testing.T) {
	...
	t.Run("drops duplicates and older", func(t *testing.T) {
		...
		x.Equal(t, rxDup, w.check(42)) // duplicate
		x.Equal(t, rxDup, w.check(41)) // older
		x.Equal(t, rxDup, w.check(1))  // much older
	})
	t.Run("seq 0 is malformed", func(t *testing.T) {
		w := rxWindow{}
		x.Equal(t, rxBeyond, w.check(0))
	})
	t.Run("wraps around", func(t *testing.T) {
		...
		x.Equal(t, rxDup, w.check(math.MaxUint32))
	})
	t.Run("lone beyond-window frame is dropped", func(t *testing.T) {
		w := rxWindow{}
		x.Equal(t, rxAccept, w.check(1))
		x.Equal(t, rxBeyond, w.check(1+wFwd+1))
	...
	t.Run("K_loud consistent beyond-window frames fail loud", func(t *testing.T) {
		...
		x.Equal(t, rxAccept, w.check(1))
		base := 1 + wFwd + 1000
		x.Equal(t, rxBeyond, w.check(base))
		x.Equal(t, rxBeyond, w.check(base+1))
	...
	t.Run("inconsistent beyond-window frames reset the run", func(t *testing.T) {
		w := rxWindow{}
		x.Equal(t, rxAccept, w.check(1))
		x.Equal(t, rxBeyond, w.check(1+wFwd+1000))
		x.Equal(t, rxBeyond, w.check(1+3*wFwd+3000)) // not within wFwd of previous
		x.Equal(t, rxBeyond, w.check(1+wFwd+1001))   // run restarts
	})
}
