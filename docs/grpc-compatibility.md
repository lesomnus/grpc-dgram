...
`*peer.Peer` to the receive ctx. All five shipped adapters do one or the
other, so over UDP and over WebTransport `p.Addr` and `p.LocalAddr` are real
socket addresses on both sides. A channel with no address of its own says so
instead:
