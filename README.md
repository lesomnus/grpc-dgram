...
| Application-written `Envelope` batching | ✅ the seam, not a policy — the core emits frames, adapters take envelopes of 1..n (each adapter's `Handle` is the 1-frame default and its `Send` — [`drpc.EnvelopeHandler`](./frame.go) — the way in; `EnvelopeSender` / `sendFrames` is its [TypeScript](./ts/src/seam.ts) twin); the library ships no batcher and [the measurement](./docs/batching-measurement.md) says why |
